...
const char fmt_wksremain_P[] PROGMEM = { "Weeks remaining: %d\n" };
const char fmt_wkspassed_P[] PROGMEM = { "Weeks counted: %d (%d%%)\n" };
const char fmt_baby_P[] PROGMEM = { "Baby!" };
...
void loop () {
    ...
    int wks_passed = PREG_WKS - wks_rem;
    float wks_pct = float(wks_passed) / float(PREG_WKS);

    strcpy_P(fbuffer, fmt_wkspassed_P);
    sprintf(sbuffer, fbuffer,
      wks_passed, int(wks_pct*100.0));
    DEBUG(sbuffer);
    ...
    if(wks_rem < 1) {
      strcpy_P(sbuffer, fmt_baby_P);
      print_text(sbuffer);
    } else {
      sprintf(sbuffer, "%dwks", wks_passed);
      print_text(sbuffer);
    }
